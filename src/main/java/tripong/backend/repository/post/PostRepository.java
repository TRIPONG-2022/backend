...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(
            value = "select * from post p where p.user_id = :userId and p.category = :postType and p.created_date between :fromDate and :endDate",
            countQuery = "select count(*) from post p where p.user_id = :userId and p.category = :postType and p.created_date between :fromDate and :endDate",
            nativeQuery = true
    )
    ...
    @Query(
            value = "select * from post p right outer join post_like l on p.id = l.post_id where l.user_id = :userId and p.category = :postType",
            countQuery = "select count(*) from post p right outer join post_like l on p.id = l.post_id where l.user_id = :userId and p.category = :postType",
            nativeQuery = true
    )
